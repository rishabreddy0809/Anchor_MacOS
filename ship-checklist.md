...
- [ ] **Canvas dependency test — a spike, not a connector.** One read-only day against a real Canvas instance, answering one question: can Canvas supply all five academic features, or not? The seam is already named. `ClassroomDataProviding` (`GoogleClassroomService.swift:40`) has five methods — `courses`, `students`, `assignments`, and `submissions` in both singular and bulk form — and any Canvas service has to satisfy every one. Prove each maps to a real endpoint returning what `AcademicSnapshot` needs: a missing count, an average grade, a grade trend across at least four graded assignments, a last-submission date, a late count. There is currently **no Canvas code in this repo at all**, so this is a genuine question rather than a formality. Stop at the answer — building the connector is a separate, much larger decision that the answer should inform.
  - **The pass/fail criterion is identity matching, not grades.** The spike fails if Canvas cannot tie a student to a Zoom participant, however good the academic data looks: a record Anchor can't attach to anyone in the call is worth nothing. Canvas exposes `login_id` and `email`, but visibility is an institution privacy setting — so test it on the *partner's own* instance, not a sandbox that happens to have it enabled. Where email is hidden the fallback is name matching, which `ZoomStudentMapper` and `ManualRosterLinks` already handle but degrade quietly at; that is a UI problem to solve before a connector, not after one.
