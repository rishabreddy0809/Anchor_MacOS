...
- [x] Automated coverage exists at all — an `AnchorTests` XCTest target, **209 passing tests** as of 2026-08-18 (the repo had none before that day). Grown in four passes, each pinning a layer where a mistake is silent rather than loud: score shaping and the three RiskLevel cut-offs; the academic escalation rules; the 16-feature vector and the cross-poll accumulators that build it (`FeatureCalculatorTests`); the Zoom redirect transports, driven over a real loopback socket (`ZoomRedirectTransportTests`); and the roster gate that decides who is scored at all (`MeetingRolesTests`). Those passes found five real defects between them — three in the feature vector, two in the redirect transports; the roster gate turned out to be already correct and is simply pinned now. See the commit messages for what and why. It still says nothing about the manual passes below, which all stay open.
