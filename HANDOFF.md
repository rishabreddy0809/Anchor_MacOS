...
- **Google's OAuth consent screen publishing status has never been checked, and if it is still "Testing" it breaks a pilot silently at day seven.** Found 2026-08-21 while trying to verify it and being unable to. **Keep two things apart, because the existing note collapses them:** *"Google verification is not needed"* is settled and true — dropping `classroom.profile.emails` left nothing restricted. That is about **review**. It says nothing about whether the consent screen has actually been **published to Production**, which is a separate switch. While an app sits in *Testing*, Google caps it at 100 explicitly-listed test users **and expires refresh tokens after seven days**. A pilot teacher would connect Classroom, have it work, and find it dead the following week with "The Google sign-in has expired" — which the app handles correctly and which would look like an Anchor bug. **The app's half is fine:** the refresh path maps Google's 400/401 to `ClassroomError.tokenExpired`, and the copy reads "The Google sign-in has expired." / "Reconnect Google Classroom in Settings." **Only the console is unknown.**
  - **Why it was not simply checked:** `console.cloud.google.com/auth/audience?project=150467027663` returns *"You need additional access"* for the Google account Chrome is signed into, and the resource manager lists **no projects at all** for it, so that account is not the one owning the OAuth client. Switching Google accounts is Rishab's to do. **One page, ten seconds, and it decides whether an outside teacher can connect Classroom at all.**
